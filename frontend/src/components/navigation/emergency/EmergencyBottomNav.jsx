...
const EmergencyBottomNav = () => {
  const location = useLocation();
  const [showMenu, setShowMenu] = useState(false);

  const mainItems = [
    { icon: LayoutDashboard, label: 'Центр', path: '/emergency/control-center' },
    { icon: Map, label: 'Карта', path: '/emergency/map' },
    { icon: AlertTriangle, label: 'Критич.', path: '/emergency/critical-zones' },
    { icon: ListOrdered, label: 'Приорит.', path: '/emergency/prioritization' },
  ];
  ...
  const moreItems = [
    ...
    { icon: User, label: 'Профиль', path: '/emergency/profile' },
  ];
  ...
  return (
    <>
      ...
      {showMenu && (
        ...
        >
          <div 
            className="absolute bottom-0 left-0 right-0 bg-gradient-to-b from-red-500 via-red-600 to-red-700 rounded-t-3xl shadow-2xl animate-slide-up max-h-[70vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
            style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
          >
            {/* Header */}
            <div className="sticky top-0 bg-gradient-to-b from-red-500 to-red-600 px-6 py-4 border-b border-white/10 backdrop-blur-xl z-10">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-white">Дополнительно</h3>
                  <p className="text-xs text-red-100 mt-0.5">Emergency функции</p>
                </div>
                <button
                  onClick={() => setShowMenu(false)}
                    ...
                    <Link
                      ...
                      className={`flex flex-col items-center justify-center space-y-2 p-4 rounded-2xl transition-all active:scale-95 min-h-[80px] ${
                        active
                          ...
                          : 'bg-white/10 text-red-50 hover:bg-white/15'
                      }`}
                    >
      ...
      <nav 
        className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-r from-red-600 via-red-600 to-red-700 border-t-2 border-red-500/30 shadow-[0_-4px_20px_rgba(0,0,0,0.3)]"
        style={{ 
      ...
      >
        <div className="grid grid-cols-5 gap-0.5 h-[70px] max-w-screen-xl mx-auto">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center space-y-1 transition-all active:scale-95 relative group ${
              showMenu ? 'text-white' : 'text-red-100'
            }`}
          >
            {/* Active indicator */}
            {showMenu && (
...
export default EmergencyBottomNav;